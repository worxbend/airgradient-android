...
- `MonitoringStartupReconciler` runs from `MainActivity` on app open and restores persisted active monitoring modes through `AirQualityMonitoringServiceController`. If restoration is rejected because configuration or notification permission is missing, it stops monitoring through the same controller path.
...
- Clearing the configured device URL from Settings must also stop monitoring through `AirQualityMonitoringServiceController` and clear monitoring runtime diagnostics; do not bypass the controller with direct service intents or direct WorkManager calls.
